...
class GeneralClient:
    def __init__(self, client_id, model, data_path, output_dir):
        ...
        self.local_data_path = os.path.join(
            data_path, "local_training_{}.json".format(self.client_id)
        )
    ...
    def preprare_local_dataset(self, generate_and_tokenize_prompt, local_val_set_size):
        if local_val_set_size > 0:
            ...
            self.local_train_dataset = (
                self.local_data["train"].shuffle().map(generate_and_tokenize_prompt)
            )
    ...
    def terminate_local_training(
        self, epoch, local_dataset_len_dict, previously_selected_clients_set
    ):
        local_dataset_len_dict[self.client_id] = len(self.local_train_dataset)
        new_adapter_weight = self.model.state_dict()
        single_output_dir = os.path.join(
            self.output_dir, str(epoch), "local_output_{}".format(self.client_id)
        )
        ...
        previously_selected_clients_set = previously_selected_clients_set | set(
            {self.client_id}
        )

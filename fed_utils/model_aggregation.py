            ...
            if k == 0:
                ...
                for key in weighted_single_weights.keys():
                    weighted_single_weights[key] = (
                        weighted_single_weights[key] * (weights_array[k])
                    )
            else:
                    ...
                    if k == 0:
                        ...
                        for key in weighted_single_weights.keys():
                            if single_weights[key].shape[0] == local_ranks[client_id]:
                                pad = ZeroPad2d(
                                    padding=(0, 0, 0, max_lora - local_ranks[client_id])
                                )
                                weighted_single_weights[key] = (
                                    pad(weighted_single_weights[key])
                                    * (weights_array[k])
                                )
                            elif single_weights[key].shape[1] == local_ranks[client_id]:
                                pad = ZeroPad2d(
                                    padding=(0, max_lora - local_ranks[client_id], 0, 0)
                                )
                                weighted_single_weights[key] = (
                                    pad(weighted_single_weights[key])
                                    * (weights_array[k])
                        ...
                        for key in single_weights.keys():
                            ...
                            if single_weights[key].shape[0] == local_ranks[client_id]:
                                ...
                                single_weights[key] = (
                                    pad(single_weights[key]) * (weights_array[k])
                                )
                    ...
                    if k == 0:
                        ...
                        for key in weighted_single_weights.keys():
                            ...
                            if heter:
                                x += 1
                                if (
                                    weighted_single_weights[key].shape[0]
                                    == local_ranks[client_id]
                                ):
                                    weighted_single_weights[key] = (
                                        weighted_single_weights[key]
                                        * (weights_array[k] * 1)
                        ...
                        for key in single_weights.keys():
                            if heter:
                                x += 1
                                if (
                                    single_weights[key].shape[0]
                                    == local_ranks[client_id]
                                ):
                                    new = [
                                        weighted_single_weights[key],
                                        single_weights[key] * (weights_array[k]) * 1,
                                    ]

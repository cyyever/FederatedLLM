...
if diff_quantity:
    ...
    while min_size < min_require_size:
        ...
        for k in range(len(category_uniques)):
            category_rows_k = remaining_df.loc[
                remaining_df["category"] == category_uniques[k]
            ]
            ...
            proportions = np.array(
                [
                    ...
                    for p, idx_j in zip(proportions, idx_partition)
                ]
            )
            proportions = proportions / proportions.sum()
            proportions = (np.cumsum(proportions) * len(category_rows_k_index)).astype(
                int
            )[:-1]
            idx_partition = [
                idx_j + idx.tolist()
                for idx_j, idx in zip(
                    idx_partition, np.split(category_rows_k_index, proportions)
                )
            ]
    ...
    shards = np.array_split(
        remaining_df_index, int(num_shards_per_clients * num_clients)
    )
    random.shuffle(shards)

    shards = [
        shards[i : i + num_shards_per_clients]
        for i in range(0, len(shards), num_shards_per_clients)
    ]
...
for client_id, idx in enumerate(idx_partition):
    ...
    with open(
        os.path.join(data_path, "local_training_{}.json".format(client_id)), "w"
    ) as outfile:
